...
from src.image_utils import extract_image_from_editor, extract_mask_from_editor
...
def build_task_instruction(task_type, custom_instruction):
    ...
    if task_type == "Custom":
        return custom_instruction if custom_instruction else "restore only the masked region naturally"
...
def gradio_inference(editor_value, task_type, custom_instruction, steps, guidance_scale, seed):
    try:
...
### Tips
- Draw a **small and precise** mask.
- If the result is unrelated, the mask is often too large or parsed incorrectly.
- For local restoration, try guidance scale around **4.5 ~ 7.0**.
"""
        )
        ...
        with gr.Row():
            with gr.Column():
                ...
                task_type = gr.Dropdown(label="Select Task", choices=list(TASK_PRESETS.keys()), value="Remove Watermark")

                custom_instruction = gr.Textbox(
                    label="Custom Instruction / Extra Requirement",
                    placeholder="Optional. Example: keep the wall texture, preserve the bicycle spoke structure",
                    lines=3,
                    value="",
                )

                with gr.Row():
                    steps = gr.Slider(minimum=10, maximum=100, value=40, step=1, label="Inference Steps")
                    guidance_scale = gr.Slider(minimum=1.0, maximum=15.0, value=5.5, step=0.5, label="Guidance Scale")

                seed = gr.Number(value=42, precision=0, label="Seed")
                run_button = gr.Button("Generate Restoration", variant="primary")

            with gr.Column():
                final_instruction = gr.Textbox(label="Final Task Instruction", lines=2)
                positive_prompt = gr.Textbox(label="Generated Positive Prompt", lines=5)
                negative_prompt = gr.Textbox(label="Generated Negative Prompt", lines=5)

                with gr.Row():
                    processed_input = gr.Image(label="Processed Input", type="pil")
                    processed_mask = gr.Image(label="Extracted Mask", type="pil")

                result_image = gr.Image(label="Restored Result", type="pil")
                comparison_image = gr.Image(label="Comparison: Input | Mask | Result", type="pil")
        ...
        run_button.click(
            fn=gradio_inference,
            inputs=[editor, task_type, custom_instruction, steps, guidance_scale, seed],
            outputs=[
                final_instruction,
                positive_prompt,
                negative_prompt,
...
if __name__ == "__main__":
    ...
    demo = build_app()
    demo.launch(share=args.share, server_name="0.0.0.0", server_port=args.server_port)
